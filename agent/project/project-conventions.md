# Project Conventions — aurora-studio
...
- **Source root** (matches `source_root` in the pipeline `config.json`): `aurora-studio`
...
- **Language:** unknown. Reference the appropriate source file extensions.
...
- `aurora-studio` is only the folder where the clone lives in the workspace — it is
  **not** part of the repository. Never write `aurora-studio/src/...` or
