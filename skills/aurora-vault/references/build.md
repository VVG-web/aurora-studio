...
Чинится после факта: `kb:repair --links` (подтягивает то, что находится по алиасу,
регистру и гомоглифам) и `kb:repair --aliases` (оставляет синоним у карточки, чьё имя с
ним совпадает, снимает у остальных). Но дешевле не создавать: обе правки потом читает
человек в `git diff`.
