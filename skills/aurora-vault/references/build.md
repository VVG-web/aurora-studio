...
Отметку ставит не разбор, а ассистент командой `build_plan.py --done <файл> --cards N`
после каждого файла. Отсюда две ошибки:

- **разобрал, но не отметил** — файл вернётся в план, и второй разбор наплодит двойников;
- **отметил, но карточек не создал** — файл выпал из плана навсегда. Это тише и опаснее:
  доля обработанного растёт, знание — нет.

Второе чинится `kb:build --reopen`: он сверяется не со счётчиком в манифесте, а с базой —
