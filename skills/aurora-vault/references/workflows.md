...
0. **Наружу уходит документ, а не кухня.** Публикация режет тело по строке-маркеру
   `<!-- ниже — производство, в чистовик не идёт -->`: «Уточнения», «Допущения», «Под
   вопросом» на страницу не попадают, сам маркер тоже. Локальный файл при этом не
   меняется — разделы производства нужны аналитику. Если страница вышла короче тела
   документа, обрезали не по маркеру: сверяйте последний абзац страницы с последним
   абзацем тела, по виду страницы это не отличить.
...
## make <тип> <задача> — производство артефакта цепочкой (основной путь)

`agent:make` — то, ради чего собиралась база. Панель: «Продуктивность» → тип → задача
своими словами. Ручной `create` (ниже) остался командой, но маршрут теперь один.

Цепочка: **обогащение** базой → **план** с вопросами к аналитику → **воркер** пишет →
**критик** правит → **Момус** проверяет утверждения. Этапы отмечены в шапке (`pipeline:`),
состояние прогона — в `Workspaces/<sid>/session.json`: после обрыва производство
продолжается с той же точки, а не с начала.

**Файл рождается сразу после обогащения**, до того как написана первая строка тела. Это
не мелочь: ответы аналитика на вопросы планировщика ложатся в «Уточнения» **по ходу**, и
обрыв посреди прогона не уносит с собой сказанное человеком.

### Анатомия готового артефакта

| Часть | Что означает | Чем испортить |
|---|---|---|
| `based_on` | карточки, на которые документ **сослался** | записать туда весь пак: сорок карточек в контексте и три ссылки в тексте значат, что документ стоит на трёх |
| «Уточнения» | вопросы планировщика с ответами | собрать их в конце, а не писать по ходу |
| «Допущения» | что предположили, **с пометкой чьё**: движка или модели | снять пометку — аналитик не знает, с кем спорить |
| «Под вопросом» | ссылки на то, чего в паке не было, с пометкой «по памяти» | принять за основание: назвать то, чего не давали, — тот же класс, что утверждение без опоры |
| `coverage` | зрелость по осям, от критика | ждать от модели чистого JSON: вывод санируется перед записью в шапку |
| `unsupported` | утверждений без опоры по Момусу | ставить `status: ready` при ненулевом |

Первые четыре живут **под маркером** `<!-- ниже — производство, в чистовик не идёт -->` —
инвариант 11. Аналитику они нужны, разработке и тестированию нет, заказчику вредны.

**Незаконченное не пропадает и не удаляется.** Брошенный прогон виден в «Проект» →
«Здоровье» → «Незаконченные»: сколько, как давно, переход к каждому. Движок их не чистит
сам — это работа человека, пусть и неоконченная, а срок автоудаления никто не подберёт
правильно, тогда как потеря необратима.

**Правило «без технологий»** — у каждого вида артефакта своё (`tech_agnostic`), не общее
на проект: у ОПЗ и проектного решения стек — предмет документа, у критериев приёмки —
чужое имя, которое устареет раньше требования. Что включено, показывает
`make:kinds --kind <тип>`.
...
**Требование заменяется только с дельтой** (инвариант 12). Для карточки
...
Отказ не препятствие, а единственный момент, когда человек ещё помнит ответ: через
полгода в базе две редакции и никакого следа, чем они различаются — разработка сверяет их
построчно и догадывается. Прочерк и пробел за ответ не считаются. На карточках других
типов дельта не требуется: иначе правка знания начнёт стоить как правка требования, и
`type: requirement` перестанут ставить.
