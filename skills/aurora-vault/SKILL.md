...
запрос уходит модели с окном пошире, а не гасит провайдера ошибкой. Настройка
«одновременно» (`AURORA_AGENT_PARALLEL`) задаёт, сколько карточек разбирать параллельно —
разбор это ожидание шлюза, а не работа машины.
...
| `agent:distill` | тезисы для карточек типа «знание»: модель пишет определение, дословный источник уезжает под тезис. Каждую проверяет Момус | `docs/knowledge-rules.md` |
