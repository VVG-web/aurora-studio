...
| `agent:make` | произвести артефакт по типу из реестра проекта: обогащение базой, план с вопросами к аналитику, воркер, критик, Момус. Этапы отмечены в шапке документа — после обрыва производство продолжается с той же точки. В задаче понимает `@путь` (файл или папка проекта), `/навык` (метод навыка — в задание), `@сервер` (MCP подключается сразу; остальные — по требованию модели); `--context ПУТЬ` — приложенное панелью (вложения лежат в `.opencode/context/`, живут две недели) | `references/workflows.md` |
...
галочки у «рассуждения»). Без рассуждений тезис пишется в одиннадцать раз быстрее, но
**не так же хорошо**: замер PRJ-A 21.09.2026 с перекрёстной проверкой — каждый тезис
судили два Момуса — показал, что тезисы без рассуждений приукрашивают («возможен
вариант» → «исключительно»), и строгий Момус находит в них втрое больше утверждений без
опоры. Выключить рассуждения у `worker` — размен качества на скорость. У Момуса (`qa`) и
критика их не выключают никогда: Момус без рассуждений не нашёл ни одного из девяти
утверждений, которые нашёл с ними.
