...
flowchart TD
  ...
  E -->|"ассистент читает источник"| F["Карточки<br>status#58; draft"]
  ...
  M -->|"задача в доверенном статусе,<br>доверенный источник,<br>ссылки принятых историй"| N["status#58; knowledge<br>+ knowledge_basis + knowledge_hash"]
  M -->|"нет основания"| O["Остаётся человеку<br>kb#58;queue → kb#58;verify"]
  O --> N

  N -->|"ctx#58;context"| P["Контекст для работы<br>только knowledge"]
  ...
  B -.->|"страница изменилась<br>sync#58;audit --drift"| T{"Карточка<br>knowledge?"}
...
| 5 | Извлечение | задание из `kb:build` → ассистент | карточки со `status: draft`, `source:`, `source_synced:` | файл разобран **целиком** | **да** |
...
| 10 | Класс доверия | `kb:trust` | статус и основание в шапке | видно, чему верить и почему | нет |
| 11 | Приёмка | `kb:trust --auto` | `status: knowledge`, `knowledge`, `owner`, `review_by`, `knowledge_basis`, `knowledge_hash` | доля knowledge выше порога `bootstrap.knowledge_threshold_pct` | **да** — иначе знание не попадёт в контекст |
...
stateDiagram-v2
  ...
  draft --> draft: человек начал править<br>или задача ещё в аналитике
  ...
  draft --> knowledge: kb#58;verify --auto<br>основание записано в карточку
  knowledge --> knowledge: kb#58;verify --refresh<br>продлить срок годности
  ...
  note right of knowledge
    ...
    knowledge_hash разойдётся с телом,
...
- **draft** — машина принесла, человек не смотрел. В контекст идёт только в режиме
  bootstrap (пока доля knowledge ниже порога) и всегда с пометкой «не считать фактом».
- **draft** — идёт работа, либо `kb:trust --by-jira` понизил карточку: связанная задача
...
- **knowledge** — знание. Обязателен `owner` и `review_by`; в `knowledge_basis` записано,
  **почему** поверили, в `knowledge_hash` — отпечаток тела на момент приёмки.
...
`kb:trust --auto` применяет их по очереди. Первое подходящее выигрывает, кроме случая,
...
|---|---|---|---|
| статус задачи | `--by-jira` | все связанные задачи в `trust_statuses` → `knowledge`; в `assumption_statuses` → `draft`; спорят — решает человек | `atlassian.jira.*_statuses` |
| происхождение | `--by-source` | источник начинается с доверенного префикса или лежит в верхней ветке вики с доверенным названием | `verify.trusted_sources`, `verify.trusted_branches` |
...
| `knowledge` мало, хотя база большая | не заданы `verify.trusted_sources` / `trust_statuses` | заполнить конфиг, затем `kb:trust --auto` |
| линтер ругается «правили после приёмки» | карточку дописали после `knowledge` | `kb:trust --refresh` либо понизить статус |
| в контекст лезет непроверенное | доля knowledge ниже порога — включён bootstrap | принять больше карточек или поднять порог осознанно |
...
sync:confluence → kb:build → [ассистент разбирает партию] → kb:build --done
   → kb:links --cards → kb:lint → kb:trust --auto → ctx:context
```

Всё остальное — `kb:moc`, `kb:trust`, `kb:repair`, `ops:trace` — улучшает качество базы,
