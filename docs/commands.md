...
Справочник собран автоматически (`kit:list`) для версии движка **1.52.0**.
...
| `kb:repair` (`fix`) | ремонт: битые ссылки, гомоглифы, легаси-frontmatter, поля вне схемы, заготовки под ссылки | скрипт | `kb_fix.py --all` | `--links --homoglyphs --retire --frontmatter --stubs --aliases --drop-alias --dupes --merge --merge-all --apply --allow-dirty --report --root` | 1.3.0 |
| `kb:dedupe` | двойники: поиск, пакетное слияние по правилу (`--merge-all`) и разбор одной пары (`--merge` «оставить» «убрать») | скрипт | `kb_fix.py --dupes` | `--links --homoglyphs --retire --frontmatter --stubs --aliases --drop-alias --all --merge --merge-all --apply --allow-dirty --report --root` | 1.3.0 |
...
| Команда | Что делает | Исполнитель | Чем | Модификаторы | С версии |
|---|---|---|---|---|---|
| `dev:qa-list` | QA движка: какие есть кейсы и сценарии, что закрыто автотестом, какие кейсы не гоняются ни разу | скрипт | `dev_qa.py --list` | `--check --gap --base --run --record --new` | 1.51.0 |
| `dev:qa-check` | целостность реестра QA: дубли номеров, ссылки на несуществующие кейсы, отставшие версии | скрипт | `dev_qa.py --check` | `--list --gap --base --run --record --new` | 1.51.0 |
| `dev:qa-gap` | что изменено в коде и чем покрыто; решение «автотест или кейс» принимает модель | скрипт+модель | `dev_qa.py --gap` | `--list --check --base --run --record --new` | 1.51.0 |
| `dev:qa-run` | прогон сценария с записью журнала: автотесты, чек-лист шагов, отчёт в Development/QA/runs/ | скрипт+модель | `dev_qa.py --run` | `--list --check --gap --base --record --new` | 1.51.0 |
| `dev:qa-new` | завести тест-кейс или сценарий из шаблона со следующим свободным номером | скрипт | `dev_qa.py --new` | `--list --check --gap --base --run --record` | 1.51.0 |
