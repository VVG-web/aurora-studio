...
| Команда | Что делает | Исполнитель | Чем | Модификаторы | С версии |
|---|---|---|---|---|---|
| `kb:build` (`build`) | извлечение карточек: план, партии и учёт — скриптом, само извлечение — моделью | скрипт+модель | `build_plan.py` | `--budget --max-files --partition --done --cards --status` | 1.0.0 |
...
| `kb:verify` (`verify, promote`) | гейт imported/draft → verified: отбор человеком, запись скриптом; `verified` — верхний статус базы | скрипт+модель | `kb_verify.py` `OWNER selector` | `--owner --months --status --refresh --apply --allow-dirty` | 1.8.0 |
...
| `ship:export` (`export`) | поставляемый документ → docx/pdf (pandoc, фирменный шаблон) | скрипт | `export_doc.py` `source` | `--format --reference --out --keep-links` | 1.5.0 |
| `ship:release` (`release`) | заморозить переданную версию: снапшот, коммит базы, дата | скрипт | `release_doc.py` `document` | `--version --date --binary --apply` | 1.9.1 |
...
| Команда | Что делает | Исполнитель | Чем | Модификаторы | С версии |
|---|---|---|---|---|---|
| `ops:stats` (`status, stats`) | дашборд здоровья базы: статусы, риски, метрики | скрипт | `aurora_stats.py` | `--json --append-metrics --report` | 1.3.0 |
| `ops:impact` (`impact`) | что зависит от карточки; `--explain` — на чём собран документ | скрипт | `kb_impact.py` `[target]` | `--explain` | 1.8.0 |
| `ops:trace` (`trace`) | трассировка: пункт ТЗ → REQ → SPEC → Jira → AC → ПМИ → приёмка | скрипт | `aurora_trace.py` | — | 1.0.0 |
