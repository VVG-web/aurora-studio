...
Справочник собран автоматически (`kit:list`) для версии движка **1.90.0**.
...
| `kb:reset` (`reset`) | обнулить базу и собрать заново: сносит из AuroraKnowledgeDB/ то, что соберётся заново; карточки неизвестного происхождения остаются, `--drop-unknown` сносит и их; откат — из git | скрипт | `kb_reset.py` | `--apply --drop-unknown --list-unknown --backup --allow-dirty` | 1.24.0 |
...
| `agent:distill` | написать тезисы для карточек типа «знание»: модель читает перенесённый текст и пишет определение, дословный источник уезжает под тезис; каждую проверяет Момус (итог: карточки-знания вместо кусков страниц) | скрипт+модель | `agent_runner.py --task distill` | `--question --mode --thread --threads --no-journal --backend --no-momus --apply --critic --limit --partition --until-done --hours --no-checkpoint` | 1.90.0 |
