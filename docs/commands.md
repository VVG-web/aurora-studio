...
Справочник собран автоматически (`kit:list`) для версии движка **1.75.0**.
...
| Команда | Что делает | Исполнитель | Чем | Модификаторы | С версии |
|---|---|---|---|---|---|
| `agent:aliases` | агент разбирает конфликты синонимов: уточняет там, где карточки разные, и откладывает человеку дубли; правит только через команды движка | скрипт+модель | `agent_runner.py --task aliases` | `--question --mode --thread --threads --no-journal --apply --critic --limit --partition --no-checkpoint` | 1.57.0 |
| `agent:build` | агент разбирает партию источников на карточки: раскадровка, границы тем, имена, отметка о разборе; тело карточек переносит движок, а не модель | скрипт+модель | `agent_runner.py --task build` | `--question --mode --thread --threads --no-journal --apply --critic --limit --partition --no-checkpoint` | 1.58.0 |
| `agent:ask` | спросить базу своими словами: движок собирает контекст, модель отвечает только по карточкам и ставит ссылку на каждое утверждение; разговор пишется в `meta/ask/` и уходит в git с базой, `--thread` продолжает его уточняющим вопросом | скрипт+модель | `agent_runner.py --task ask` | `--question --mode --thread --threads --no-journal --apply --critic --limit --partition --no-checkpoint` | 1.63.0 |
