...
def main() -> int:
    ...
    # «Разное» объявляется ВСЕГДА, даже пустым. Условное объявление прятало группу от
    # отчёта и от уборки: когда последняя неприкаянная карточка находила свою группу,
    # «Разное» исчезало из плана — и старый файл карты оставался на диске навсегда,
    # ссылаясь на карточки, которых в базе уже нет.
    ...
    # Карта, которую перестали порождать, обязана уйти с диска. Группа пустеет обычным
    # ходом дела: карточки «Разного» разошлись по разделам, последняя карточка группы
    # уехала в архив. Файл же оставался и продолжал ссылаться на то, чего в базе нет, —
    # причём вечно: следующий прогон его не переписывает, потому что не порождает.
    # Трогаем только своё: карту с шапкой «ФАЙЛ ГЕНЕРИРУЕТСЯ» — руками писанное не наше.
    dropped = 0
    if a.apply:
        for name, _n2, items in planned:
            if items:
                continue
            path = os.path.join(MOC_DIR, re.sub(r"[^\w\- ]", "", name).strip()
                                .replace(" ", "-") + ".md")
            if os.path.isfile(path) and machine_made(path):
                os.remove(path)
                dropped += 1
                print(f"  убрана карта без карточек: {path}")
    if dropped:
        print(f"Карт удалено (группа опустела): {dropped}")
    if not a.apply:
