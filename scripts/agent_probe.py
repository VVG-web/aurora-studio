...
def backends() -> list:
    ...
    out = []
    # Чат спрашиваем только у чатовых шлюзов. Шлюз, поднятый под вектора, чат-модели не
    # держит: спросив его чатом, проверка получила бы отказ и объявила живой сервер
    # сломанным. Вектора проверяются своей пробой — у них своё кольцо.
    for b in [x for x in cfg["backends"] if x.get("chat", True)]:
