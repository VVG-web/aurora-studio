...
def cmd_check() -> int:
    ...
    # Сверки «кейс отстал от версии движка» здесь больше нет, и это не упрощение.
    # За всё время она не нашла ни одного настоящего расхождения — только требовала
    # поднять число в полусотне файлов после каждого минора. Поднятое не глядя число
    # хуже отсутствующего: оно утверждает, что кейс перечитали.
    #
    # Настоящее отставание ловит автотест `qa corpus does not describe a removed engine`:
    # снятая команда или снятый статус в живом кейсе роняют прогон. Это проверяемый
    # факт, а не срок давности.
    stale: list = []
    if stale:
