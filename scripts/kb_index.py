...
def first_sentence(text: str, limit: int = 120) -> str:
    ...
    for line in text.splitlines():
        ...
        # Снимаем разметку ссылок, оставляя слова. Обрезка по длине рвала `[[…]]`
        # пополам, в описание попадали открывающие скобки без закрывающих, и следующая
        # ячейка таблицы закрывала их своей разметкой — выходила ссылка на имя-обрубок.
        # Она не ведёт никуда по построению: линтер звал оглавление отставшим, а
        # `kb:index` писал тот же обрубок заново. Описанию ссылки и не нужны — на
        # карточку ведёт первая колонка строки.
        s = re.sub(r"\[\[([^\]|#]+?)(?:#[^\]|]*)?(?:\\?\|([^\]]*))?\]\]",
...
def index_like(old: str, rows: list, elsewhere: set = frozenset()) -> bool:
    ...
    """
    # Заготовка, которую написал сам движок при разворачивании проекта. Пометку в неё
    # начали ставить только в 1.76.3, и без этой строки старые проекты навсегда остаются
    # с одиннадцатью «рукотворными» оглавлениями, которых никто не писал.
    if STUB in old:
        return True
...
def main() -> int:
    ...
    # держит не отсутствие флага, а решение про чужой текст.
    if written and not a.apply:
        print(f"\n(dry-run) Ничего не записано. Записать {written}: повторите с --apply.")
    elif written:
        print(f"\n✅ Обновлено индексов: {written}")
    elif stale:
        print("\nЗаписывать нечего: оглавления этих разделов движок считает рукотворными. "
              "Дальше — решение человека:\n"
              "  · отдать разделы движку: `kb:index --force --apply` (прежний текст "
              "заменится, заголовок и введение сохранятся);\n"
              "  · либо дописать недостающие карточки в оглавления руками.")
    else:
        print("\nОглавления в порядке: всё на месте, делать нечего.")
