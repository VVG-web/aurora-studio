...
def main() -> int:
    ...
    if EMB.DEAD:
        # Шлюз векторов не ответил: замер сделан по словам. В историю его не пишем — рядом
        # с прошлыми замерами он показал бы «падение качества», которого нет.
        print("\n⚠️  Шлюз векторов не отвечал (" + ", ".join(sorted(EMB.DEAD)) + ") — замер "
              "сделан поиском по словам и в историю не записан.")
        return 0
    if a.apply:
        h = history()
