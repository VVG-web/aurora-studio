...
BARE_CODE_RE = re.compile(r"^[A-ZА-ЯЁ][A-ZА-ЯЁ0-9]*(?:[.\-_][A-ZА-ЯЁ0-9]+)*[.\-_]\d+$")


def is_doc_code(name: str) -> bool:
    """Имя целиком — код документа: «PRJ.SYS.ERD-006», «US-3.6.14». → да/нет.

    `split_doc_code` снимает код, когда за ним идёт название: «US-3.6.14 Отправка». А
    голый код она не узнаёт — и он попадал в «понятия без карточки», где выглядел
    сущностью. Заводить под него карточку значит плодить бумагу вместо знания: по правилу
...
def derived_card(path: str) -> bool:
    ...
    Второй признак — `built: machine`. Происхождение бывает пустым: карточка написана
    движком, а список источников у неё не заполнен (потерян при переносе, стёрт ремонтом).
    Тогда её снова принимало за рукописный справочник — и на живой базе карточка
    «Варианты расписания запуска алгоритмов» встала в план источником, критик каждый
    оборот отклонял её имя («это имя файла, а не название карточки»), отметка не
